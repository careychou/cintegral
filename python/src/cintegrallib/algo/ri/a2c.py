...
class A2C:
    ...
    def _build_net(self):
        ...
        with tf.variable_scope('disc_net'):
            ...
            self.disc_reward = -1 * tf.nn.softmax_cross_entropy_with_logits(labels=self.disc_z,
                                                                            logits=self.disc) - self.p_z
        ...
        with tf.variable_scope('loss'):
            ...
            if self.a2c:
                self.entropy_loss = -tf.reduce_sum(self.action_probs * tf.log(self.action_probs + 1e-10), axis=1,
                                                   keepdims=True) # penalize over-dominate actions and encourage exploration
                ...
                self.loss1 = 0.1 * self.policy_loss + 1 * self.advantage - 1 * self.entropy_loss
            else:
    ...
    def learn(self):
        ...
        if self.prioritized:  # use prioritized replay
            ...
            _, self.cost, _, self.disc_cost, self.adv_cost, self.policy_cost, self.entrop_cost = self.sess.run([
                self._train_op,
                self.loss,
                self._train_disc_op,
                self.disc_loss,
    ...
    def p_z_info(self, obs, skill):
        return self.sess.run([self.p_z],
                             feed_dict={self.s: obs, self.q_skill: skill})
